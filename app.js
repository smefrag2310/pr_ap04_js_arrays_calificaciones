 ...
 * @type {HTMLButtonElement}
 */
const ordenAsc= document.querySelector("#btnOrdenAsc");

/**
 * Botón para ordenar de mayor a menor.
 * @type {HTMLButtonElement}
 */
const ordenDesc= document.querySelector("#btnOrdenDesc");
...
function agregarNota() {
  ...
  if(isNaN(numero) || numero < 0 || numero > 10){
    mostrarMensaje("La nota debe ser un número entre 0 y 10.");
    return;
  }
  notas.push(numero);
  notaInput.value = "";
...
function pintarResumen() {
  ...
  // TODO: construir un texto resumen y asignarlo a txtResumen.textContent
    ...
    const max= calcularMax(notas);
    const min= calcularMin(notas);
    const aprobados= contarAprobados(notas);
    const suspensos= notas.length - aprobados;
    resumen.innerHTML= `- Total: ${notas.length} <br>
